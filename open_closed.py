...
for mpc in MPC:
    ...
    for t in range(T):
        ...
        c[t]     = c_b_closed + mpc * (primary_deficit[t] + (r - anmp[t]) * d[t])
        if t < T - 1:
...
k        = 10.0
w        = (1 - alpha) * A * k ** alpha
a_b      = (c_b_closed - w) / (s - g) #calibrated so c_b_open = c_b_closed
c_b_open = w + a_b * (s - g)
...
for mpc in MPC:
    ...
    for t in range(T):
        c[t] = c_b_open + mpc * primary_deficit[t]
        if t < T - 1:
...
ax2.plot(years, consumption_closed[0.5] - c_b_closed, color='#053769', label='Consumption, closed (MPC=0.5)')
ax2.plot(years, consumption_open[0.5]   - c_b_open,   color='#ff5e1a', label='Consumption, open (MPC=0.5)')
